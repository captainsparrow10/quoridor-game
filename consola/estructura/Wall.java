...
public class Wall {
    ...
    public Boolean colocarVallaHorizontal(int x, int y1, int y2,int y3, String[][] tablero, int walls, TableroGUI table2) {
        if (walls > 0) {
            if (tablero[x][y1 - 1] == "x" && tablero[x][y2] == "x" && tablero[x][y1] == "x") {
                ...
                JOptionPane.showMessageDialog(null, "Vaya colocada correctamente", "mensaje de acierto", JOptionPane.DEFAULT_OPTION);
                return true;
            } else {
    ...
    public Boolean colocarVallaVertical(int y, int x1, int x2,int x3, String[][] tablero, int walls, TableroGUI table2) {
        if (walls > 0) {
            System.out.println("x2 :" + x2 + "x1 : " + x1 + "y : " + y);
            if (tablero[x2 - 1][y] == "x" && tablero[x2][y] == "x" && tablero[x1][y] == "x") {
                tablero[x1][y] = "v";
                tablero[x2][y] = "v";
                tablero[x3][y] = "v";
